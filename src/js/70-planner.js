...
function homeBlocked(){
  ...
  if (!fieldMode && !geo) return 'Home is fixed on the practice field. Load a field image or import a plan first.';
  if (tel.armed || !tel.on_ground) return 'Land and disarm before moving home.';
  return '';
}
function setHomePick(on, msg){
  homePick = on; $('pgHome').setAttribute('aria-pressed', on ? 'true' : 'false');
  $('homeMsg').textContent = msg || (on ? 'Click the map where you will stand and take off. Press Esc or the button again to cancel.' : '');
}
function moveHome(hx, hz){
  var why = homeBlocked(); if (why){ setHomePick(false, why); return; }
  ...
  startDrill(drill.id); setHomePick(false, 'Home moved. The drone now sits at the new home.'); computePlan(); geoNote();
}
$('pgHome').addEventListener('click', function(e){
  e.target.blur(); if (homePick){ setHomePick(false); return; }
  var why = homeBlocked(); if (why){ setHomePick(false, why); return; }
  calib = null; $('calibBox').hidden = true; $('calibMsg').textContent = ''; setHomePick(true); drawMap();
});
window.addEventListener('keydown', function(e){ if (e.code === 'Escape' && homePick) setHomePick(false); });
