...
var geo = null, view2d = { cx: 0, cz: -32, span: 150 }, track = [], calib = null;
...
function ll2xz(lat, lon){ return [(lon - geo.lon0)*geo.mLon, -(lat - geo.lat0)*geo.mLat]; }
function xz2ll(x, z){ return [geo.lat0 - z/geo.mLat, geo.lon0 + x/geo.mLon]; }
...
function fitView(){
  ...
  if (fieldRect){ pts.push([-fieldRect.w/2, fieldRect.cz - fieldRect.h/2]); pts.push([fieldRect.w/2, fieldRect.cz + fieldRect.h/2]); }
...
function computePlan(){
  var cam = CAMERAS[+$('pCam').value], alt = num('pAlt', 5, 120, 40), fo = num('pFront', 10, 95, 75)/100, so = num('pSide', 10, 95, 70)/100;
  var speed = num('pSpeed', 1, 8, 5), dir = num('pDir', 0, 179, 0)*Math.PI/180, endurance = num('pBatt', 5, 45, 20);
  var W = cam.sw*alt/cam.f, L = cam.sh*alt/cam.f, P = { cam: cam, alt: alt, speed: speed, W: W, L: L, dir: dir, mode: planMode };
  P.gsd = cam.sw*alt*100/(cam.f*cam.iw); P.spacing = W*(1 - so); P.trig = L*(1 - fo); P.interval = P.trig/speed;
  P.lines = []; P.shots = []; P.wps = []; P.area = 0; P.dist = 0; P.time = 0; P.endurance = endurance; P.ok = false;
  var i, prev = [0, 0];
  if (planMode === 'route'){
    if (route.length >= 1){
      ...
      route.forEach(function(r){
        P.wps.push({ x: r.x, y: r.alt, z: r.z, photo: r.act === 'photo', hold: r.act === 'hover' ? 5 : (r.act === 'photo' ? 1.5 : 0) });
        P.dist += Math.hypot(r.x - prev[0], r.z - prev[1]); prev = [r.x, r.z]; climb += Math.abs(r.alt - lastAlt); lastAlt = r.alt; hold += r.act === 'hover' ? 5 : (r.act === 'photo' ? 1.5 : 0);
        if (r.act === 'photo') P.shots.push([r.x, r.z, 0]);
      });
      P.dist += Math.hypot(prev[0], prev[1]); P.wps.push({ x: 0, y: lastAlt, z: 0 });
      P.time = P.dist/speed + route.length*4 + hold + climb/2 + route[0].alt/2.5 + lastAlt/0.6 + 10; P.ok = true;
    }
    ...
    for (var k = 0; k < nL && k < 400; k++){
      ...
      var v1 = Math.min.apply(null, hits), v2 = Math.max.apply(null, hits); if (v2 - v1 < 1) continue;
    ...
    P.time = P.dist/speed + P.lines.length*2*4 + alt/2.5 + alt/0.6 + 10; P.ok = P.lines.length > 0;
  }
  plan = P; renderPlanOut(); renderWpTable(); drawMap();
}
...
function drawMap(){
  ...
  if (fieldRect){ a = w2m(-fieldRect.w/2, fieldRect.cz - fieldRect.h/2); mx.drawImage(fieldCanvas, a[0], a[1], fieldRect.w*sc, fieldRect.h*sc); }
...
map.addEventListener('pointerdown', function(e){
  ...
  if (!calib) ptsList().forEach(function(p, i){ var d = Math.hypot(p[0] - w[0], p[1] - w[1]); if (d < bd){ bd = d; best = i; } });
...
map.addEventListener('pointerup', function(e){
  ...
  if (planMode === 'route') route.push({ x: w[0], z: w[1], alt: num('pAlt', 5, 120, 40), act: 'none' }); else poly.push(w);
  computePlan();
});
...
$('pgWhole').addEventListener('click', function(){
  var R = fieldRect || { w: 60, h: 44, cz: -32 }, x = R.w/2, z1 = R.cz - R.h/2, z2 = R.cz + R.h/2;
  setPlanMode('area'); poly = [[-x, z1], [x, z1], [x, z2], [-x, z2]]; computePlan();
});
